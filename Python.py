...
# -----------------------------{Exercice Complémentaire bis}--------------------
# Sur une telle matrice, faire la somme de tous les éléments; 3<= M[i,j] <= 7
...
# -------------------{Exercice Complémentaire quatro}-------------------
# Problème des villes
...
def Pbvilles(N):
    ...
    b = 6000
    c = 3000
    M = np.array([29/42, 1/6, 1/7], [1/2, 2/5, 1/3], [1/7, 1/10, 5/12])
    P = np.array([a, b, c])
    T = P.T
    for i in range(N):
