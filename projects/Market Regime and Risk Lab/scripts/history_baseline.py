...
feature_columns = ["Five-day volatility"]
...
learn = df[(df["Date"] >= "2018-01-01") & (df["Date"] <= "2022-12-31")] # Filter the rows for the specified date range.
...
# Create the historical-volatility baseline.

predictions = X_valid["Five-day volatility"] # Generate predictions for the selected data.

print("Mean Squared Error (MSE):", mean_squared_error(y_valid, predictions)) # Calculate the mean squared error.
print("R-squared (R2):", r2_score(y_valid, predictions)) # Calculate the R-squared score.
print("Mean Absolute Error (MAE):", mean_absolute_error(y_valid, predictions)) # Calculate the mean absolute error.

# Create the historical-volatility baseline.

predictions_test = X_test["Five-day volatility"] # Generate predictions for the selected data.

print("Mean Squared Error (MSE) on test set:", mean_squared_error(y_test, predictions_test)) # Calculate the mean squared error.
print("R-squared (R2) on test set:", r2_score(y_test, predictions_test)) # Calculate the R-squared score.
print("Mean Absolute Error (MAE) on test set:", mean_absolute_error(y_test, predictions_test)) # Calculate the mean absolute error.
...
predictions_2022 = X_test_2022["Five-day volatility"]
...
predictions_2023 = X_test_2023["Five-day volatility"]
...
predictions_2024 = X_test_2024["Five-day volatility"]
...
predictions_2025 = X_test_2025["Five-day volatility"]
